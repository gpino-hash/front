...
export default function ClientDashboardPage() {
  ...
  const btnOutline = "border border-slate-200 dark:border-zinc-700 text-slate-600 dark:text-zinc-300 hover:bg-slate-50 dark:hover:bg-zinc-800 rounded-lg px-3 py-1.5 text-xs font-medium transition-colors flex items-center justify-center gap-1 flex-1 sm:flex-initial";
  const btnDanger = "border border-red-200 dark:border-red-900 text-red-600 dark:text-red-400 hover:bg-red-50 dark:hover:bg-red-950/40 rounded-lg px-3 py-1.5 text-xs font-medium transition-colors flex items-center justify-center gap-1 flex-1 sm:flex-initial";
    ...
    <div className="flex-1 overflow-y-auto p-4 md:p-8 bg-slate-50 dark:bg-zinc-950">
      <div className="mx-auto max-w-6xl space-y-6">
        ...
        <div className="grid grid-cols-2 gap-3 lg:grid-cols-4">
          <StatCard label="Activas" value={String(stats.active)} detail={stats.pending > 0 ? `${stats.pending} pendientes` : "Al día"} detailColor={stats.pending > 0 ? "text-amber-600 dark:text-amber-400" : "text-emerald-600 dark:text-emerald-400"} icon="pending_actions" iconBg="bg-blue-50 dark:bg-blue-950/40" iconColor="text-primary" />
          <StatCard label="Completados" value={String(stats.completed)} detail="Total" icon="check_circle" iconBg="bg-emerald-50 dark:bg-emerald-950/40" iconColor="text-emerald-600 dark:text-emerald-400" />
          <StatCard label="Gastado" value={formatPrice(stats.spent)} detail="Servicios completados" icon="payments" iconBg="bg-purple-50 dark:bg-purple-950/40" iconColor="text-purple-600 dark:text-purple-400" />
          <StatCard label="Guardados" value="5" detail="Ver lista" detailColor="text-primary" icon="favorite" iconBg="bg-orange-50 dark:bg-orange-950/40" iconColor="text-orange-500" />
        </div>
        ...
        <div className="grid grid-cols-1 gap-6 lg:grid-cols-3">
          ...
          <div className="lg:col-span-2 space-y-4">
            ...
            {/* List */}
            {filtered.length === 0 ? (
              <div className="flex flex-col items-center justify-center rounded-xl border-2 border-dashed border-slate-200 dark:border-zinc-800 bg-white dark:bg-zinc-900 py-14 text-center">
                <span className="material-symbols-outlined text-[40px] text-slate-300 dark:text-zinc-700 mb-3">inbox</span>
                <p className="text-sm font-medium text-slate-600 dark:text-zinc-300 mb-1">
                  {filter === "completed" ? "Sin historial todavía" : "Sin solicitudes activas"}
                </p>
                <p className="text-xs text-slate-400 dark:text-zinc-500 mb-4 max-w-xs">Creá tu primera solicitud y recibí presupuestos de profesionales.</p>
              ...
              <div className="space-y-3">
                {filtered.map((req) => {
                  ...
                  return (
                    <div key={req.id} className="bg-white dark:bg-zinc-900 rounded-xl border border-slate-200 dark:border-zinc-800 shadow-sm hover:shadow-md transition-shadow">
                      <div className="flex flex-col sm:flex-row">
                        {/* Category icon */}
                        <div className="flex items-center justify-center bg-slate-50 dark:bg-zinc-800/50 shrink-0 h-14 sm:h-auto sm:w-[72px] border-b sm:border-b-0 sm:border-r border-slate-100 dark:border-zinc-800">
                          <span className="material-symbols-outlined text-slate-400 dark:text-zinc-500 text-[24px]">{icon}</span>
                        </div>
                        ...
                        <div className="flex-1 p-3.5 min-w-0">
                          <div className="flex items-start justify-between gap-2 mb-1">
                            <div className="min-w-0">
                              <h4 className="text-sm font-bold text-slate-900 dark:text-white truncate">{req.serviceName}</h4>
                              <p className="text-xs text-slate-500 dark:text-zinc-400">
                                {req.serviceCategory}
                                {req.proposals > 0 && req.status === "PENDING_QUOTE" && (
                                  <> · <span className="text-amber-600 dark:text-amber-400 font-medium">{req.proposals} propuestas</span></>
                                )}
                              </p>
                            </div>
                          ...
                          {/* Meta */}
                          <div className="flex flex-wrap items-center gap-x-3 gap-y-0.5 text-xs text-slate-500 dark:text-zinc-400 mb-2">
                            <span className="flex items-center gap-1">
                              <span className="material-symbols-outlined text-[13px] text-slate-400 dark:text-zinc-500">calendar_today</span>
                              {formatDate(req.scheduledDate, req.scheduledTime)}
                            </span>
                            <span className="flex items-center gap-1">
                              <span className="material-symbols-outlined text-[13px] text-slate-400 dark:text-zinc-500">{provider ? "person" : req.proposals > 0 ? "group" : "person_search"}</span>
                            ...
                            {req.quotedPrice && (
                              <span className="flex items-center gap-1 font-semibold text-slate-700 dark:text-zinc-200">
                                <span className="material-symbols-outlined text-[13px] text-slate-400 dark:text-zinc-500">payments</span>
                        ...
                        {/* Actions */}
                        <div className="flex sm:flex-col gap-1.5 p-3 sm:p-3.5 sm:pl-0 sm:border-l sm:border-slate-100 dark:sm:border-zinc-800 sm:w-[120px] shrink-0 border-t sm:border-t-0 justify-end">
          ...
          <div className="space-y-5">
            ...
            <div className="bg-white dark:bg-zinc-900 rounded-xl border border-slate-200 dark:border-zinc-800 p-5 shadow-sm">
              <h3 className="text-sm font-bold text-slate-900 dark:text-white mb-3">Categorías populares</h3>
              <div className="grid grid-cols-2 gap-2">
                {[
                  { icon: "plumbing", label: "Plomería", href: "/servicios/plomeria" },
                  { icon: "electrical_services", label: "Electricidad", href: "/servicios/electricidad" },
                  { icon: "yard", label: "Jardinería", href: "/servicios/jardineria" },
                  { icon: "cleaning_services", label: "Limpieza", href: "/servicios/limpieza" },
                ].map((cat) => (
                  <Link key={cat.label} href={cat.href} className="group flex flex-col items-center gap-1.5 p-3 rounded-lg border border-slate-100 dark:border-zinc-800 bg-slate-50 dark:bg-zinc-800/50 hover:border-primary/30 hover:bg-blue-50/60 dark:hover:bg-blue-950/20 transition-colors">
                    <span className="material-symbols-outlined text-slate-400 dark:text-zinc-500 group-hover:text-primary transition-colors text-[22px]">{cat.icon}</span>
                    <span className="text-xs font-medium text-slate-700 dark:text-zinc-200">{cat.label}</span>
                  </Link>
                ))}
              </div>
              <Link href="/servicios" className="mt-3 flex w-full items-center justify-center rounded-lg border border-dashed border-slate-200 dark:border-zinc-700 py-2 text-xs font-medium text-slate-400 dark:text-zinc-500 hover:text-primary hover:border-primary/30 transition-colors">
                Ver todas
              </Link>
            </div>
            ...
            <div className="bg-white dark:bg-zinc-900 rounded-xl border border-slate-200 dark:border-zinc-800 p-5 shadow-sm">
              <div className="flex items-center justify-between mb-3">
                <h3 className="text-sm font-bold text-slate-900 dark:text-white">Proveedores recientes</h3>
                ...
                {[PROVIDERS_MAP.prov1, PROVIDERS_MAP.prov2, PROVIDERS_MAP.prov5].map((p) => (
                  <div key={p.id} className="flex items-center justify-between">
                    <div className="flex items-center gap-2.5">
                      <div className="w-8 h-8 rounded-full bg-primary/10 dark:bg-primary/20 flex items-center justify-center text-primary font-bold text-xs">{p.name.charAt(0)}</div>
                      <div>
                        ...
                        <div className="flex items-center gap-0.5">
                          ...
                          <span className="text-[11px] text-slate-500 dark:text-zinc-400">{p.rating} ({p.reviews})</span>
                        </div>
                      </div>
                    </div>
                    <button className="p-1.5 rounded-full text-slate-400 dark:text-zinc-500 hover:bg-slate-100 dark:hover:bg-zinc-800 hover:text-primary transition-colors">
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 backdrop-blur-sm p-4">
          <div className="w-full max-w-sm bg-white dark:bg-zinc-900 rounded-xl p-6 shadow-xl animate-scale-in border border-slate-200 dark:border-zinc-800">
            <div className="flex items-center gap-3 mb-4">
              ...
              <h3 className="text-base font-bold text-slate-900 dark:text-white">Cancelar solicitud</h3>
            </div>
            <p className="text-sm text-slate-500 dark:text-zinc-400 mb-1">¿Estás seguro que querés cancelar?</p>
            <p className="text-sm font-semibold text-slate-800 dark:text-zinc-100 mb-5 bg-slate-50 dark:bg-zinc-800 rounded-lg px-3 py-2">{cancelTarget.serviceName}</p>
            <div className="flex gap-3">
              <button onClick={() => setCancelId(null)} className="flex-1 border border-slate-200 dark:border-zinc-700 text-slate-700 dark:text-zinc-200 rounded-lg py-2 text-sm font-medium hover:bg-slate-50 dark:hover:bg-zinc-800 transition-colors">Volver</button>
...
function StatCard({ label, value, detail, detailColor = "text-slate-400 dark:text-zinc-500", icon, iconBg, iconColor }: {
